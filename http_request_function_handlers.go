...
func handleRequest(requestS RequestStruct) (*Response, error) {
  request, err = http.NewRequest(requestS.chosenMethod.String(), requestS.url, requestS.byteRequestBody)
	if err != nil {
		return nil, err
	}
...
func (m mainModel) setContentsOnRequestResponse() {
  ...
  response, err := handleRequest(newRequestStruct)
  if err != nil {
